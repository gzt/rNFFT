...
void nfft_2dtest(){
  ...
  rand_unit_complex(p.f_hat,p.N_total);
  for(int i = 0; i < 7; i++) Rprintf("Here is x: %f\n",(p.x[i]));
  ...
  for(int i = 0; i < 7; i++) Rprintf("Here is ndft vector f: %f + %f i\n",crealf(p.f[i]), cimagf(p.f[i]));
  ...
  /* NFFT(vpr_complex)(p.f, K, "nfft, vector f (first few entries)"); */
    for(int i = 0; i < 7; i++) Rprintf("Here is nfft vector f: %f + %f i\n",crealf(p.f[i]), cimagf(p.f[i]));
  ...
  NFFT(adjoint_direct)(&p);
  for(int i = 0; i < 7; i++) Rprintf("Here is adjoint ndft vector f_hat: %f + %f i\n",crealf(p.f_hat[i]), cimagf(p.f_hat[i]));

  /* NFFT(vpr_complex)(p.f_hat, K, "adjoint ndft, vector f_hat (first few entries)"); */

  /** approx. adjoint and show the result */
  NFFT(adjoint)(&p);
    for(int i = 0; i < 7; i++) Rprintf("Here is nfft vector f_hat: %f + %f i\n",crealf(p.f_hat[i]), cimagf(p.f_hat[i]));
